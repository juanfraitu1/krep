#!/usr/bin/env python3
"""Run nucmer verification on candidate k-mer pairs for segmental duplications.

usage: run_nucmer_pairs.py <pairs.tsv> <genome.fa> <sd.bed> <out_prefix> [n_pairs]

Filters pairs to 10k-100k distance, samples n_pairs positive (both in SD) and
n_pairs negative, extracts windows, runs nucmer A-vs-B, and outputs verified BED.
"""
...
print(f"pairs 10k-100k: positive {len(pos_pairs)}, negative {len(neg_pairs)}", file=sys.stderr)
...
seq = ''.join(seq)
print(f"loaded {name} {len(seq)} bp", file=sys.stderr)

# write A and B windows to separate multi-fastas
...
with open(fa_a, 'w') as out_a, open(fa_b, 'w') as out_b:
    for label, p1, p2, is_pos in selected:
        ...
        out_b.write(f">{label}\n")
        out_b.write(seq[b:e] + "\n")
print(f"wrote {fa_a} and {fa_b}", file=sys.stderr)

# run nucmer A vs B (not self; default finds best alignment per pair)
prefix = f"{out_prefix}_nucmer"
cmd = ["nucmer", "--nosimplify", "-p", prefix, fa_a, fa_b]
print("running", ' '.join(cmd), file=sys.stderr)
subprocess.run(cmd, check=True)

# run show-coords to get tabular output
...
# parse coords: keep alignments between matching pair label
# show-coords -r -l -T columns (1-based, tab separated):
# 1 S1, 2 E1, 3 S2, 4 E2, 5 LEN1, 6 LEN2, 7 %IDY, 8 LEN R, 9 LEN Q, 10 TAGS (ref_tag query_tag)
verified = []
with open(coords_out) as f:
    # skip header lines (4 lines typically)
    for _ in range(4):
        next(f, None)
    for line in f:
        cols = line.rstrip('\n').split('\t')
        ...
        if len(tags) != 2:
            continue
        tagr, tagq = tags[0], tags[1]
        if tagr != tagq:
            continue
        m = re.match(r'^(pos_\d+|neg_\d+)$', tagr)
        if not m:
            continue
        s1, e1, s2, e2 = int(cols[0]), int(cols[1]), int(cols[2]), int(cols[3])
        len1, len2, idy = int(cols[4]), int(cols[5]), float(cols[6])
        if len1 < 500 or idy < 90.0:
            continue
        rec = next((x for x in selected if x[0] == tagr), None)
        if rec is None:
            continue
        label, p1, p2, is_pos = rec
        # convert coords to chr1 positions
...
print(f"wrote {bed_out}", file=sys.stderr)
